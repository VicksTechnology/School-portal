...
<div class="row">
  <div class="col-sm-12">
    ...
    <?php
		...
		$select_student_from_model = $this->db->get_where('student' , array('parent_id' => $parent_id, 'student_id' => $student_id))->result_array(); 
	...
	<?php if(get_settings('report_template') == 1):?>
				...
				<table width="100%">
				  <tr>
					<td width="38%" >
						...
						<p class="pull-letf" style="margin-top:-10px; font-size:12px"><strong>YEAR: </strong> <strong style="text-decoration:underline"><?=get_settings('session');?></strong>&nbsp;<strong>TERM: </strong>
		...
		<?php endif;?>	
			...
			<?php if(get_settings('report_template') == 'udemy'):?>
				...
				<table width="100%" border="0">
				  <tr>
					<td align="center"><strong>REPORT SHEET FOR <?php if($term == 1) echo '1ST TERM'; elseif($term == 2) echo '2ND TERM'; else echo '3RD TERM';?> <?=get_settings('session')?> ACADEMIC SESSION</strong></td>
				  </tr>
				</table>
				...
				<table width="100%" height="100" border="1">
				  ...
				  <tr>
					...
					<td>&nbsp;<?=get_settings('session')?></td>
				  </tr>
				  <tr>
					<td><strong>&nbsp;DAY(S) SCHOOL OPEN</strong></td>
					<td>&nbsp;
				    <?=$this->db->get_where('attendance', array('session' => $running_year, 
					'term' => $term))->num_rows(); ?></td>
					<td width="265"><strong>&nbsp;DAY(S) PRESENT:</strong></td>
					<td>&nbsp;<?=$this->db->get_where('attendance', array('session' => $running_year, 'term' => $term,
						'student_id' => $student_id,'session' => get_settings('session'), 'status' => 1))->num_rows(); 
						?></td>
					<td><strong>&nbsp;DAY(S) ABSENT:</strong></td>
					<td><strong>&nbsp;</strong><?=$this->db->get_where('attendance', array('session' => $running_year, 'term' => $term,
						'student_id' => $student_id,'session' => get_settings('session'), 'status' => 2))->num_rows(); 
						?></td>
			      </tr>
				</table>
				<table width="100%"  border="1">
				...
				<table width="100%" border="1">
				  ...
				  <!--<tr>
					...
					<td colspan="2">&nbsp;<?=get_settings('session')?></td>
				  </tr>-->
				</table>
